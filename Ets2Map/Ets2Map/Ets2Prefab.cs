...
namespace Ets2Map {
    public class Ets2Prefab {
        ...
        private void Parse() {
            ...
            // Versions 22 and 21 are slightly different
            if (version == 22) {
                Parse22();
            } else {
                Parse21();
        ...
        private void Parse22() {
            ...
            var nodeOffset = BitConverter.ToInt32(Stream, 48);
            var navCurveOffset = BitConverter.ToInt32(Stream, 52);
            var off3 = BitConverter.ToInt32(Stream, 56);
            var off4 = BitConverter.ToInt32(Stream, 60);


            for (int navCurve = 0; navCurve < navCurves; navCurve++) {
                var curveOff = navCurveOffset + navCurve * 132;
